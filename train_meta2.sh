...
for i in "${!depth[@]}";do
    ...
    ## train on the VisA dataset
        base_dir=${depth[i]}_${n_ctx[j]}_${t_n_ctx[0]}_meta_mvtec
        save_dir=./checkpoints/${base_dir}/2024_08_20/
        CUDA_VISIBLE_DEVICES=${device} python train_meta2.py --dataset mvtec \
        --train_data_path /mnt/IAD_datasets/mvtec_anomaly_detection \
        --json_path /mnt/IAD_datasets/mvtec_anomaly_detection/processed_meta3.json \
        --save_path ${save_dir} \
        --features_list 6 12 18 24 --image_size 518  --batch_size 8 --print_freq 1 \
        --epoch 30 --save_freq 1 --depth ${depth[i]} --n_ctx ${n_ctx[j]} --t_n_ctx ${t_n_ctx[0]}
    wait
    done
done
