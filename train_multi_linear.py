...
def train(args):
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_dataloader):
            ...
            # Map specie_name to category index
            specie_index = []
            for name in specie_name:
                for category, names in categories.items():
                    if name in names:
                        specie_index.append(category_labels[category])
                        break

            subtype = torch.tensor(specie_index, dtype=torch.long).to(device)
            ...
            # num_text_chunks = text_features.shape[0]
            # if batch_size != num_text_chunks:
            #     min_size = min(batch_size, num_text_chunks)
            #     image_features = image_features[:min_size]
            #     label = label[:min_size]
            #     subtype = subtype[:min_size]
            ...
            specie_pred_probs = text_probs[:, 2:6]
            specie_logits = torch.argmax(specie_pred_probs, dim=-1).float()
            print("specie_logits shape:", specie_logits.shape)
            print("subtype shape:", subtype.shape)
            # specie_logits = model.specie_classifier(image_features)
            # specie_logits = specie_logits[:subtype.size(0)]
            specie_loss = loss_ce(specie_logits, subtype)
            ...
            optimizer.zero_grad()
            (loss + image_loss + specie_loss).backward()
            # no specie loss
